...
# Each section gets its own try/catch — a failure redeploying the frontend must not
# skip checking the benchmark. One shared try/catch around the whole script used to
# do exactly that: a FATAL in section 2 silently meant section 3 never ran that cycle.
...
try {
    ...
    Log "FATAL in backend section: $_"
}
...
try {
    ...
    if (-not $tunnelUp) {
        ...
        if ($newUrl) {
            ...
            # A previous run's worktree can be left dangling (e.g. this same script
            # crashing mid-deploy) — prune first and use a unique path so a stale
            # registration can never block `git worktree add` again.
            git worktree prune 2>&1 | Out-Null
            $tmpWorktree = "$env:TEMP\watchdog-gh-pages-$(Get-Date -Format 'yyyyMMddHHmmss')"
...
try {
    ...
    Log "FATAL in benchmark section: $_"
}
